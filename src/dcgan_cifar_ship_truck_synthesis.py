...
#dataroot = '/common/users/aj611/pytorch/celeba'
dataroot = '/common/users/aj611/pytorch/cifar10_images/cifar/ship-truck/'
...
image_size = 64
#image_size = 32 # the cifar10 images size is 32x32
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        # save 5000 images from this ship-truck class
        ...
        if ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
            for j in range(0, 5000):
                ...
                fake = netG(noise)
                name = "%d.jpeg"%(j)
                vutils.save_image(fake, name)
       ### new
        #new_arr = np.transpose(img_list[-1],(1,2,0)).transforms.ToPILImage(mode='RGB')
       ...
       ### new
        #new_arr = np.transpose(img_list[-1],(1,2,0)).transforms.ToPILImage(mode='RGB')
        #vutils.save_image(fake, "3.jpeg")
#transform = transforms.ToPILImage()
#x_image = transform(fake)
...
#x_image.save("2.jpeg")
        ...
        if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
            ...
            vutils.save_image(fake, "progression.jpeg")
...
#plt.imshow(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True).cpu(),(1,2,0))
#print(real_batch[0])
print(np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True).cpu()).size())
...
plt.show()
print(img_list[-2].size())


#new_arr = np.transpose(img_list[-1],(1,2,0)).transforms.ToPILImage(mode='RGB')
from PIL import Image
transform = transforms.ToPILImage()
x_image = transform(np.transpose(img_list[-1],(0,1,2)))
y_image = transform(np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True), (0,1,2)))
#x_image = transform(np.transpose(img_list[-1]))
